...
class FIREEVENT:
    def __init__(self, cfg):
        ...
        # MTBS 
        self.BurnAreaPolys = ee.FeatureCollection("projects/ee-vishalned/assets/ems-2022")

        print('Number of total events: ',self.BurnAreaPolys.size().getInfo())
    ...
    def query_wildfire_events_info(self):
        ...
        year_filter = ee.Filter.eq("EMS_year", f"{self.cfg.YEAR}")
        ...
        # firename list
        nameList = polyFiltered.aggregate_array("emsr_id").getInfo()
        ...
        for event_id in nameList:
            year = self.cfg.YEAR
            poly = polyFiltered.filter(ee.Filter.eq("emsr_id", event_id))
            
            
            
            
            roi = poly.union(ee.ErrorMargin(100)).geometry().bounds()    
            print(event_id, roi.getInfo())
            ...
            eventName = f"EU_{year}_{event_id}"
