...
def get_archived_viirs_progression(prg_roi, year=2020, start_date=None, end_date=None):
    base_AF_SUOMI_VIIRS = ee.FeatureCollection(f"users/omegazhangpzh/NRT_AF/SUOMI_VIIRS_C2_Global_Archived_{year}")
    base_AF_J1_VIIRS = ee.FeatureCollection(f"users/omegazhangpzh/NRT_AF/J1_VIIRS_C2_Global_Archived_{year}")
    AF_BASE = base_AF_SUOMI_VIIRS.merge(base_AF_J1_VIIRS)
    ...
    #   // print("before filtering: " + viirs_af.size().getInfo())
    if start_date is None: start_date = f"{year}-01-01"
    if end_date is None: end_date = f"{year}-12-31"
    viirs_af = (viirs_af
                    .filter(ee.Filter.gte("julian_day", ee.Date(start_date).getRelative('day', 'year')))
                    .filter(ee.Filter.lte("julian_day", ee.Date(end_date).getRelative('day', 'year')))
                )
